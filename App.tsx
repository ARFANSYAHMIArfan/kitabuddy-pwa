...
import { 
  loginUser, signInWithGoogle, logoutFirebase, onAuthStatusChanged,
  getUsers, updateUserRole, addUser, deleteUser,
...
const App: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    if (!isOnline) {
      setLoginError("Sambungan internet diperlukan untuk log masuk Google.");
      return;
    }
    ...
    try {
      ...
      if (maintenanceMode && !maintenanceBypass && !googleUser.role.toLowerCase().includes('admin')) {
        setLoginError("Sistem dalam penyelenggaraan. Hanya pentadbir dibenarkan akses.");
        setIsLoggingInWithGoogle(false);
        return;
      }

      setUserRole(googleUser.role);
      setUserName(googleUser.name);
      setUserEmail(googleUser.email);
      setUserPhoto(googleUser.photoURL || '');
      setView(ViewState.MENU);
    } catch (error: any) {
      console.error('Google Sign-In Error:', error);
      if (error?.code === 'auth/popup-closed-by-user') {
        setLoginError('Tetingkap log masuk ditutup sebelum selesai.');
      } else if (error?.code === 'auth/unauthorized-domain') {
        setLoginError(`Domain (${window.location.hostname}) belum dibenarkan di Firebase Console. Sila benarkan domain ini di Firebase Authentication > Settings > Authorized domains.`);
        setShowDomainHelp(true);
      } else if (error?.code === 'auth/network-request-failed') {
        setLoginError('Ralat sambungan rangkaian ke Firebase.');
      } else {
        setLoginError(error?.message || 'Gagal log masuk dengan Google. Sila cuba lagi.');
      }
    } finally {
      setIsLoggingInWithGoogle(false);
    }
      ...
      <div className="max-w-4xl w-full animate-fade-in-up">
        <div className="bg-white rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[500px]">
          ...
          <div className="md:w-7/12 p-10 md:p-16 flex flex-col justify-center bg-white">
             <div className="mb-6">
               <h3 className="text-2xl font-bold text-[#3D405B] mb-1 font-fredoka">Akaun Pengguna</h3>
               <p className="text-slate-400 text-sm font-nunito">Sila log masuk dengan Google atau ID pengguna anda.</p>
             </div>
             ...
             <button 
               ...
               className="w-full mb-6 bg-white hover:bg-slate-50 text-slate-700 font-bold py-3.5 px-4 rounded-2xl border-2 border-slate-200 hover:border-slate-300 shadow-sm hover:shadow transition-all flex items-center justify-center gap-3 active:scale-[0.99] disabled:opacity-60 cursor-pointer"
             >
                ...
                <span className="text-sm md:text-base font-bold text-[#3D405B]">
                  {isLoggingInWithGoogle ? 'Menyambung ke Google...' : 'Log Masuk dengan Google'}
                </span>
             </button>

             <div className="relative flex py-2 items-center mb-6">
                <div className="flex-grow border-t border-slate-200"></div>
                <span className="flex-shrink mx-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Atau ID Manual</span>
                <div className="flex-grow border-t border-slate-200"></div>
             </div>
                ...
                {showDomainHelp && (
                  <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-xs text-amber-800 space-y-2">
                    <div className="flex items-center gap-2 font-bold text-amber-900">
                      <Info size={16} /> Domain Perlu Dibenarkan di Firebase:
                    </div>
                    <div className="font-mono bg-white p-2 rounded-lg border border-amber-200 select-all font-bold text-slate-700">
                      {window.location.hostname}
                    </div>
                    <p className="text-[11px] leading-relaxed text-amber-700">
                      Sila masukkan domain di atas ke dalam Firebase Console &gt; Authentication &gt; Settings &gt; Authorized domains.
                    </p>
                  </div>
                )}
          ...
          <div className="flex items-center gap-3">
             {userPhoto ? (
               <img 
                 src={userPhoto} 
                 alt={userName} 
                 className="w-12 h-12 rounded-2xl border-2 border-[#81B29A] object-cover shadow-sm flex-shrink-0"
                 referrerPolicy="no-referrer"
               />
             ) : (
               <div className="w-12 h-12 rounded-2xl bg-[#81B29A]/20 text-[#81B29A] flex items-center justify-center font-black font-fredoka text-xl flex-shrink-0">
                 {userName ? userName.charAt(0).toUpperCase() : 'K'}
               </div>
             )}
             <div>
                <h2 className="text-xl md:text-2xl font-black text-[#3D405B] font-fredoka leading-tight">
                  Hai, {userName || 'Kawan'}! 👋
                </h2>
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                  <span className="text-[11px] bg-[#81B29A] text-white font-bold px-2 py-0.5 rounded-full capitalize">
                    {userRole || 'Pelajar'}
                  </span>
                  {userEmail && (
                    <span className="text-xs text-slate-400 font-medium truncate max-w-[160px] md:max-w-xs">
                      {userEmail}
                    </span>
                  )}
                </div>
          ...
          <div className="flex gap-2">
            {userRole.toLowerCase().includes('admin') && (
                <button 
                  ...
                  className="p-3 bg-slate-200 hover:bg-slate-300 text-[#3D405B] rounded-full transition-colors"
                  title="Admin Console"
                >
                    <Settings size={22} />
                </button>
            ...
            <button 
              ...
              className="p-3 bg-red-100 hover:bg-red-200 text-red-500 rounded-full transition-colors"
              title="Log Keluar"
            >
                <LogOut size={22} />
            </button>
